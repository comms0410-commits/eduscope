...
- 웹에서 OpenAI API 키를 현재 탭 세션에만 설정하는 AI 시장자료 분석
- 데스크톱, 태블릿, 모바일 반응형 UI
...
웹의 **API 설정**에서도 사용자가 직접 키를 입력할 수 있습니다. 이 키는 `sessionStorage`에만 저장되어 탭을 닫으면 삭제되며, 분석 요청 시 Netlify Function으로 전달됩니다. 공개·공용 기기에서는 사용하지 않는 것을 권장합니다. Netlify 환경 변수 `OPENAI_API_KEY`가 설정되어 있으면 서버 키가 우선 사용됩니다.
